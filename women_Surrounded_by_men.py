...
# Define a threshold for many men
threshold_men = 3  # You can adjust this value based on your requirement
...
while webcam.isOpened():
    ...
    # Check if one woman is surrounded by many men
    elif women_count == 1 and men_count >= threshold_men:
        # Display the alert on the frame
        cv2.putText(frame, "ALERT: Woman Surrounded by Many Men!", (10, 60), cv2.FONT_HERSHEY_SIMPLEX,
                    0.9, (0, 0, 255), 3)
            ...
            responseData = sms.send_message(
                {
                    "from": "Vonage APIs",
                    "to": "+919047114805",  
                    "text": f"Alert: A woman is surrounded by {men_count} men. Location: Lat {latitude}, Long {longitude}",
                }
            )
